//Correct bug of persistence of chrono inside DOM
function chronoFix() {
  // On coupe les clicks sur TOUS les chrono
  document.querySelectorAll(".chrono").forEach(el => {
    el.style.pointerEvents = "none";
  });

  // On réactive SEULEMENT le chrono dans la slide "present"
  const present = document.querySelector(".reveal .slides section.present");
  if (!present) return;

  present.querySelectorAll(".chrono").forEach(el => {
    el.style.pointerEvents = "auto";
  });
}

// Quand Reveal est prêt
Reveal.on("ready", chronoFix);
Reveal.on("slidechanged", chronoFix);
Reveal.on("overviewshown", chronoFix);
Reveal.on("overviewhidden", chronoFix);
